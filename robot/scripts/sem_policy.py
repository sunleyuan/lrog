...
# 预设的物品列表
targets = ["chair", "bed", "plant", "toilet", "monitor", "sofa"]
...
i_run_once_has_been_run2 = False

def speak2 (text):
    global i_run_once_has_been_run2

    if i_run_once_has_been_run2:
        return

    engine.say(text)
    engine.runAndWait()

    i_run_once_has_been_run2 = True
...
class SLAMTrainer(Agent):
    def __init__(self, args) -> None:
        ...
        if args.visualize or args.print_images:
            ...
            self.legend = cv2.imread("/lrognav_node/scripts/legend.png")
    ...
    def act(self, observations: Observations) -> Dict[str, int]:
        ...
        if self.l_step == 0:
            ...
            if self.args.visualize or self.args.print_images:
                self.vis_image = vu.init_vis_image(category_to_id[observations['objectgoal'][0]], None,self.legend)
        ...
        # depth = depth.reshape(480, 640, 1)  # 重新添加单一通道维度
        state = np.concatenate((rgb, depth), axis=2).transpose(2, 0, 1)
        ...
        # if self.stop or (self.l_step % self.args.num_local_steps == self.args.num_local_steps - 1 and self.l_step < 26):
            ...
            print("sself.g_action= ", self.g_action)
        ...
        global_item = self.g_action.cpu().numpy()
         # random walking
        # global_item = torch.randint(0, 4, (1,)).item()
        print("self.global_goals2: ", global_item)
        ...
        if self.local_map[cn, :, :].sum() != 0.:
            ...
            found_goal = 1
            speak("Found target, I am approaching to it!")
        # ------------------------------------------------------------------
    ...
    def _plan(self, planner_inputs):
        ...
        if stop and planner_inputs['found_goal'] == 1:
            ...
            action = 0
            speak2("I reached the target object less than one meter, task successful")
    ...
    def _preprocess_obs(self, obs, use_seg=True):
        ...
        depth = obs[:, :, 3:4]
        # 调整图像大小到 (480, 640, 1)
        depth = cv2.resize(depth, (640, 480))  # cv2.resize的参数顺序是宽度x高度
        depth = depth.reshape(480, 640, 1)  # 重新添加单一通道维度
    ...
    def _get_sem_pred(self, rgb, depth, use_seg=True):
        ...
        if (self.l_step % self.args.num_local_steps == self.args.num_local_steps - 1) or (self.l_step==0):
            print("estimate room type!!!!")
            # 加载预训练的CLIP模型
            ...
            # 房间类型描述
            room_types = [
                "bathroom", "bedroom", "childs_room", "closet", "corridor",
                "dining_room", "empty_room", "exercise_room", "garage", "home_office",
                "kitchen", "living_room", "lobby", "pantry_room", "playroom",
                "staircase", "storage_room","television_room", "utility_room"
            ]

        
            img_rgb_clip =  Image.fromarray(img_rgb)
            # img_rgb_clip = img_rgb_clip.resize((112, 112))  # 调整为较小的尺寸

            # 处理图片
            image = preprocess(img_rgb_clip).unsqueeze(0).to(device)

            # 准备文本描述
            text_descriptions = [clip.tokenize(f"a photo of a {room}") for room in room_types]
            text_descriptions = torch.cat(text_descriptions).to(device)
            ...
            with torch.no_grad():
                ...
                # 计算相似度
                logits_per_image, _ = model(image, text_descriptions)
            ...
            self.last_estimation = [(room_types[index], prob) for index, prob in zip(top_indices, top_probs)]

            # 将PIL图像转换为OpenCV图像
            # img_cv2 = self.rgb_vis

        

         # 使用存储的估计结果
        if self.last_estimation is not None:
          
            for i, (room, prob) in enumerate(self.last_estimation):
                        # 在图像上添加文本
                ...
                text = f"{room}: {prob:.2f}"
                position = (10, offset + 30 * i)
                cv2.putText(self.rgb_vis, text, position, font, font_scale, color, thickness, line_type)

        # self.rgb_vis=cv2.cvtColor(self.rgb_vis, cv2.COLOR_RGB2BGR)
            ...
            # cv2.imshow("sem.png",semantic_img)
            # cv2.waitKey()
    ...
    def _visualize(self, inputs, action):
        ...
        self.vis_image = vu.init_vis_image(self.goal_name, str(action), self.legend)
        ...
        # 放置三个图像
        self.vis_image[50:530, :640, :] = self.rgb_vis       # rgb_vis 在左侧
        
        

        depth_vis_color = cv2.cvtColor(self.depth_vis, cv2.COLOR_GRAY2BGR)
        # 加载您的深度图像
     
        # depth_vis_color = np.repeat(self.depth_vis, 3, axis=2)  # 将单通道复制到三通道
        

        self.vis_image[50:530, 640:1280, :] = depth_vis_color # depth_vis 在中间

        self.vis_image[50:530, 1280:, :] = sem_map_vis  # sem_map_vis 在右侧
        ...
        agent_arrow = vu.get_contour_points(pos, origin=(640+640, 50), size=10)
        color = (int(color_palette[11] * 255),
                 int(color_palette[10] * 255),
                 int(color_palette[9] * 255))
        cv2.drawContours(self.vis_image, [agent_arrow], 0, color, -1)

        original_height, original_width = self.vis_image.shape[:2]
        ...
        # 使用cv2.resize调整图像尺寸
        self.vis_image = cv2.resize(self.vis_image, (new_width, new_height), interpolation=cv2.INTER_AREA)
