...
class SemanticMapperModule(nn.Module):
    def __init__(self, cfg):
        ...
        self.room_activation = get_activation_fn(self.cfg.MODEL.room_activation)


    def forward(self, x):
        embedding = self.encoder(x)
        object_preds = self.object_activation(self.object_decoder(embedding))
        # print("self.object_activation=",self.object_activation)
        room_preds = self.room_activation(self.room_decoder(embedding))
        # print("self.room_activation=",self.room_activation)

        # room_preds = nn.Tanh(self.room_decoder(embedding))
        # print("room_preds=",room_preds)
        area_preds = None
        if self.area_decoder is not None:
            area_preds = self.area_activation(self.area_decoder(embedding))
        return object_preds, area_preds, room_preds
    ...
    def infer(self, x, do_forward_pass=True, input_maps=None, avg_preds=True):
        if do_forward_pass:
            object_preds, area_preds, room_preds = self(x)
        else:
        ...
        if avg_preds:
            ...
            if area_preds is not None:
                outputs = (object_preds + area_preds + room_preds) / 3.0
            return outputs
        else:
            return object_preds, area_preds, room_preds
    ...
    def undo_memory_opts(self, batch):
        inputs, labels, inputs_room, labels_room = batch
        inputs["semmap"] = inputs["semmap"].float()
        inputs_room["semmap"] = inputs_room["semmap"].float()
        labels["semmap"] = labels["semmap"].float()
        labels_room["semmap"] = labels_room["semmap"].float
        labels["object_pfs"] = labels["object_pfs"].float() / 1000.0
        labels_room["object_pfs"] = labels_room["object_pfs"].float()/1000.0
        if "area_pfs" in labels:
            labels["area_pfs"] = labels["area_pfs"].float() / 1000.0
            labels_room["area_pfs"] = labels_room["area_pfs"].float() / 1000.0
        return (inputs, labels, inputs_room, labels_room)

    def batch_step(self, batch):
        inputs, labels, inputs_room, labels_room = batch
        
        input_maps = inputs["semmap"]
        input_maps_room =inputs_room["semmap"]
        # print("input_maps_room=",input_maps_room.shape)
        # y_room = labels_room["object_pfs"]
        # print("y_room=",y_room.shape)

        object_preds, area_preds, room_preds = self(input_maps)
        # print("room_preds=",room_preds.shape)
        losses = {}
        if self.cfg.MODEL.output_type == "map":
            # object_preds - (B, N + 2, H, W)
            # Ignore free-space, wall predictions
            y_hat = object_preds[:, 2:]
            y_hat_room = room_preds[:,2:]
            y = labels["object_pfs"][:, 2:]
            y_room = labels_room["object_pfs"][:, 2:]
            mask = labels["loss_masks"][:, 2:]
            mask_room = labels_room["loss_masks"][:, 2:]
            loss = self.object_loss_fn(y_hat, y)
            loss_room = self.object_loss_fn(y_hat_room, y_room)
            print("y_hat_room=",y_hat_room,y_hat_room[10][0])
            # Evaluate predictions only on mask = 1 regions
            mask_sum = mask.sum(dim=3).sum(dim=2) + 1e-16  # (b, 1)
            mask_sum_room = mask_room.sum(dim=3).sum(dim=2) + 1e-16  # (b, 1)

            loss = (loss * mask).sum(dim=3).sum(dim=2) / mask_sum  # (b, c)
            loss_room = (loss_room * mask_room).sum(dim=3).sum(dim=2) / mask_sum_room
            loss = loss.mean()
            loss_room = loss_room.mean()
            # loss = loss + loss_room
            losses["object_pf_loss"] = loss.item()
            losses["room_pf_loss"] = loss_room.item()
        ...
        if area_preds is not None:
            ...
            loss = loss + area_pf_loss + loss_room
    ...
    def convert_to_data_parallel(self):
        ...
        self.room_decoder = nn.DataParallel(self.room_decoder)
        if self.area_decoder is not None:
...
class Trainer:
    ...
    def train(self):
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            for batch_id, (inputs, labels, inputs_room, labels_room) in enumerate(train_loader):
                num_updates += 1
                inputs = {k: v.to(self.device) for k, v in inputs.items()}
                inputs_room = {k_room: v_room.to(self.device) for k_room, v_room in inputs_room.items()}
                ...
                for key_room in [
                        ...
                        if key_room in labels_room and labels_room[key_room] is not None:
                            labels_room[key_room] = labels_room[key_room].to(self.device)
                ...
                # Forward pass
                batch = (inputs, labels, inputs_room, labels_room)
                ...
                if num_updates % cfg.LOGGING.eval_interval == 0:
                    ...
                    if total_batches * self.cfg.OPTIM.batch_size < val_size:
                        total_batches += 1
                    # ---------------------------------------------------------#
                    for inputs, labels, inputs_room, labels_room in tqdm.tqdm(val_loader, total=total_batches):
                        inputs = {k: v.to(self.device) for k, v in inputs.items()}
                        for key in [
                            "object_pfs",
                            "loss_masks",
                        ...
                        inputs_room = {k: v.to(self.device) for k, v in inputs_room.items()}
                        for key_room in [
                            "object_pfs",
                            "loss_masks",
                            "dirs",
